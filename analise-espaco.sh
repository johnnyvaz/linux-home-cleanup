...
du -sb "$HOME_DIR"/*/ 2>/dev/null | sort -rn | head -20 | while read size dir; do
...
find "$HOME_DIR" -type f -size +${MIN_SIZE_MB}M 2>/dev/null | \
    xargs -I{} stat --format="%s %Y %n" {} 2>/dev/null | \
    sort -rn | head -30 | while read size mtime filepath; do
        size_human=$(format_size "$size")
...
find "$HOME_DIR" -type f -size +50M -atime +${OLD_DAYS} 2>/dev/null | \
    xargs -I{} stat --format="%s %X %n" {} 2>/dev/null | \
    sort -rn | head -20 | while read size atime filepath; do
        size_human=$(format_size "$size")
